...
# Cria lista de tentativas de login falhas
tentativas_login_falha = defaultdict(list)

# Configurar constantes de proteçao de força bruta
MAX_TENTATIVAS_LOGIN = 3
TEMPO_BLOCK_MIN = 1

# Função de verifica id bloqueado por exesso de tentativas
def is_blocked(ip):
    if ip in tentativas_login_falha:
        tentativas = tentativas_login_falha[ip]
        # Remover tentativas antigas
        tentativas = [tentativa for tentativa in tentativas if detetime.now() - tentativas < temedelta(minutos = TEMPO_BLOCK_MIN)]
        tentativas_login_falha[ip] = tentativas

        # Se o numero de tentativas erradas exeder o limite, bloqueia o ip!
...
def login():
    # Obtem o IP do user para verificação
    ip = request.remote_addr
    if is_blocked(ip):
        return jsonify({"erro":"Execesso de tentativas erradas. Tente novamente mais tarde"}),429
